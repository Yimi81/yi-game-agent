...
from yi_game_agent.memory import MemoryBase
from yi_game_agent.message import Msg
from yi_game_agent.serialize import serialize, deserialize
from yi_game_agent.manager import ModelManager
from yi_game_agent.constants import Embedding
...
class TemporaryMemory(MemoryBase):
    ...
    def add(
        self, memories: Union[Sequence[Msg], Msg, None], embed: bool = False
    ) -> None:
        """
    ...
    def delete(self, index: Union[Iterable, int]) -> None:
        ...
        if self.size() == 0:
            logger.warning(
                "The memory is empty, and the delete operation is " "skipping.",
            )
            return
        ...
        if isinstance(index, list):
            ...
            if len(invalid_index) > 0:
                logger.warning(
                    f"Skip delete operation for the invalid " f"index {invalid_index}",
                )

            self._content = [_ for i, _ in enumerate(self._content) if i not in index]
        else:
